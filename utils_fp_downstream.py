...
class TestbedDataset(InMemoryDataset):
    def __init__(self, root='tmp', dataset='train', task='bbbp',
        ...
        # if os.path.isfile(self.processed_paths[0]) and os.path.isfile(self.processed_paths[1]) and os.path.isfile(self.processed_paths[2]):
        #     if dataset == 'train':
        #         self.data, self.slices = torch.load(self.processed_paths[0])
        #     elif dataset == 'valid':
        #         self.data, self.slices = torch.load(self.processed_paths[1])
...
def save(self, dataset, path):
    ...
    for i in range(len(dataset)):    
        ...
        GCNData = DATA.Data(x=torch.Tensor(features),
                            ...
                            pubchem = torch.Tensor(pubchem),
                            maccs=torch.Tensor(maccs),
                            erg=torch.Tensor(erg)
                            )
