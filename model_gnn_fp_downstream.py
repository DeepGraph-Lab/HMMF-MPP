...
class DPMoE(nn.Module):
    ...
    def compute_balance_loss(self, pi, K):
        """
        pi: the gating probability after softmax
        """
        N, M = pi.shape

        topk_idx = torch.topk(pi, K, dim=-1).indices

        O = torch.zeros_like(pi).scatter_(1, topk_idx, 1.0)

        density_selection = O.mean(dim=0)
        density_proxy = pi.mean(dim=0)

        balance_loss = (density_selection * density_proxy).sum() / M

        return balance_loss


    def forward_with_lb_loss(self, data):
        x, edge_index, batch, y ,edge_attr, w = data.x, data.edge_index, data.batch, data.y, data.edge_attr, data.w


        path1_logits = self.gating1(gmp(x,batch))

        path1_logits = torch.softmax(path1_logits, dim=-1)
        topk_idx = torch.topk(path1_logits, self.p1_num, dim=-1).indices
        path1_logits_gumbel = torch.zeros_like(path1_logits).scatter_(-1, topk_idx, 1.0)
        ...
        path2_logits = torch.softmax(path2_logits, dim=-1)
        topk_idx = torch.topk(path2_logits, self.p2_num, dim=-1).indices
        path2_logits_gumbel = torch.zeros_like(path2_logits).scatter_(-1, topk_idx, 1.0)
        ...
        p1_loss = self.compute_balance_loss(path1_logits, self.p1_num)
        p2_loss = self.compute_balance_loss(path2_logits, self.p2_num)

        return out, y, w, out_loss, y_loss, w_loss, p1_loss, p2_loss
