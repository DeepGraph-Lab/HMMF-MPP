...
def train(model, device, data_loader, optimizer, epoch, beta, gamma, DPMoE_variant):
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        elif DPMoE_variant == 'with_lb_loss':
            out, y, w, out_loss, y_loss, w_loss, p1_loss, p2_loss = model.forward_with_lb_loss(data)
        ...
        if DPMoE_variant == 'weighted_sum':
            ...
            loss = nn.BCELoss(weight=w_loss,reduction='mean')(pred_loss,y_loss) + beta*p1_loss + gamma*p2_loss
...
def predicting(model, device, data_loader, beta, gamma, DPMoE_variant):
    model.eval()     
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p1_num', default=6, type=int) # for lb_loss
    parser.add_argument('--p2_num', default=4, type=int) # for lb_loss
    parser.add_argument('--beta', type=float, default=50.0)  # for lb_loss, range [1e−3,1e2]
    parser.add_argument('--gamma', type=float, default=10.0)  # for lb_loss, range [1e−3,1e2]
    parser.add_argument('--DPMoE_variant', default='with_lb_loss', type=str,
                        choices=['weighted_sum', 'with_lb_loss'])
    ...
    for epoch in range(epochs+1):
        train_pred,train_y,test_loss= train(model, device, train_data_loader, optimizer, epoch + 1, beta, gamma, DPMoE_variant)
        train_losses.append(test_loss)
        valid_pred,valid_true,val_loss= predicting(model,device,valid_data_loader, beta, gamma, DPMoE_variant)
        valid_losses.append(val_loss)
        test_pred,test_true,test_loss = predicting(model,device,test_data_loader, beta, gamma, DPMoE_variant)
    ...
    test_pred,test_true,loss = predicting(model, device,test_data_loader,beta, gamma, DPMoE_variant)
